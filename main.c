...
void inputPassportID(Person_ID* id) {
    ...
    if (format == FORMAT_SINGLE_NUMBER) {
        ...
        id->number = 0;
        clearInputBuffer();
    } else if (format == FORMAT_SPACE_SEPARATED) {
        ...
        switch (choice) {
            ...
            case 9: {
                ...
                for (size_t i = 0; i < people.size; i++) {
                    char* personStr = people.data[i].toString(&people.data[i]);
                    char* paymentStr = getPaymentString(&people.data[i]);
                    if(personStr && paymentStr) {
                        printf("%s | %s\n", personStr, paymentStr);
                    }
                    free(personStr);
                    free(paymentStr);
                }
